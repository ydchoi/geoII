...
\noindent \textbf{(4)} We derive the energy with the heuristic shown in class.
Multiply both sides of the equation by $u$ and taking the integral with respect to
$x$,
\eQb
\int \partial_t u u dx - \int \triangle u u dx = \int u^2 - u^4 dx 
\eQe
which simplifies to
\eQb
\dfrac{1}{2} \dfrac{d}{dt} \int u^2 dx &=& - \int |\triangledown
 u|^2 dx - \int u^2 - u^4 dx.
\eQe
Taking the time integral with the above equation,
\eQb
\int u^2(T) dx &=& \int u_0^2 dx - \int_{0}^{T} |\triangledown 
u|^2 dx dt - \int_{0}^{T} \int 
|u|^2 dx dt + \int_{0}^{T} \int |u|^4 dx dt .
\eQe
...
time of existence $T^*$. Then, for some constant $C$, 
\eQb
...
\textbf{(8)} Suppose $u_0 \geq 0$. Note that $\Phi$ is non-negative
everywhere. Then, as for each $t,x,y$,
by 7, for each $t,x,y$,
\eQb
u(t,x,y-xt) &=& v(t,x,y) = \Phi(t,x,y)* u_0 (x,y) \geq 0
\eQe
